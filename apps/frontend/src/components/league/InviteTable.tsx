import { CreatedByCell } from "@/components/league/CreatedByCell";
...
export const InviteTable = ({ leagueSlug }: { leagueSlug: string }) => {
  const { data: invites = [] } = trpc.invite.getAll.useQuery({ leagueSlug });
  return (
    <Table>
                  ...
                  onClick={() =>
                    void navigator.clipboard
                      ...
                      .then(() =>
                        toast({
                          title: "Link copied!",
                          description: "Auto join link copied to clipboard",
                        }),
                      )
                  }
                >
