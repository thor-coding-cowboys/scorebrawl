...
export const SeasonTeamStanding = () => {
  ...
  const { data = [], isLoading: isLoadingStanding } = trpc.seasonTeam.getStanding.useQuery({
    leagueSlug,
    seasonSlug,
  });
  const { data: avatars, isLoading: isLoadingAvatars } = trpc.avatar.getBySeasonTeamIds.useQuery(
