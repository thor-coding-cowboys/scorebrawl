import { createChartConfig, getAllChartKeys } from "@/components/season/charts/utils";
...
export const PointProgression = () => {
  ...
  const { data: season } = trpc.season.findBySlug.useQuery({ leagueSlug, seasonSlug });
  const { data: seasonPlayers } = trpc.seasonPlayer.getAll.useQuery({ leagueSlug, seasonSlug });
  const { data } = trpc.seasonPlayer.getPointProgression.useQuery({ leagueSlug, seasonSlug });
  const { data: latestMatch } = trpc.match.getLatest.useQuery({
    leagueSlug,
    seasonSlug,
  });
