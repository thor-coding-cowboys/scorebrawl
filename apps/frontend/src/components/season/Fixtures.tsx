...
export const FixtureButton = ({ fixture }: { fixture: Fixture }) => {
  ...
  const { mutate } = trpc.match.createFixtureMatch.useMutation();
  const { data: matchResults } = trpc.match.getById.useQuery(
    {
      leagueSlug,
      seasonSlug,
...
export const useFixturesRounds = () => {
  const { leagueSlug, seasonSlug } = useSeason();
  const { data: fixtures, isLoading: isLoadingFixtures } = trpc.season.getFixtures.useQuery({
    leagueSlug,
    seasonSlug,
  });

  const { data: players, isLoading } = trpc.seasonPlayer.getAll.useQuery({
    leagueSlug,
    seasonSlug,
  });
