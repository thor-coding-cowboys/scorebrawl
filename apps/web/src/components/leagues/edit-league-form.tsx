...
type EditLeagueFormValues = z.infer<typeof editLeagueSchema>;
...
export function EditLeagueForm({
	...
	const deleteLogoMutation = useMutation(trpc.organization.deleteLogo.mutationOptions());
	...
	// Check slug availability with debounced value (only if slug changed)
	const { data: slugCheck, isLoading: isCheckingSlug } = useQuery({
		queryKey: ["checkSlug", debouncedSlug, league.slug],
		queryFn: async () => {
			if (!debouncedSlug || debouncedSlug === league.slug) return null;
		...
		enabled: !!debouncedSlug && debouncedSlug.length > 0 && debouncedSlug !== league.slug,
		staleTime: 0,
		gcTime: 0,
	...
	const handleRemoveLogo = (event: React.MouseEvent) => {
		...
		if (logoPreview && logoPreview !== league.logo) {
	...
	const onSubmit = async (values: EditLeagueFormValues) => {
		setApiError("");
		...
		try {
			...
			if (result.error) {
				throw new Error(result.error.message || "Failed to update league");
			}
			...
			if (selectedLogo) {
				try {
					...
					toast.error("League updated but failed to upload logo");
				}
			} else if (logoDeleted && league.logo) {
				try {
					await deleteLogoMutation.mutateAsync();
				} catch (logoErr) {
					console.error("Failed to delete logo:", logoErr);
					toast.error("League updated but failed to delete logo");
				}
			}
			...
			setApiError(
				err instanceof Error ? err.message : "Failed to update league. Please try again."
			);
		} finally {
