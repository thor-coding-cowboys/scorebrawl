...
type EditSeasonFormValues = z.infer<typeof editSeasonSchema>;
...
export function EditSeasonForm({ isOpen, onClose, onSuccess, season }: EditSeasonFormProps) {
	const trpc = useTRPC();
	...
	const { data: countInfo } = useQuery({
		queryKey: ["season-count-info", season?.slug],
		queryFn: async () => {
			if (!season) return { matchCount: 0, teamCount: 0, playerCount: 0 };
			return await trpcClient.season.getCountInfo.query({ seasonSlug: season.slug });
		},
		enabled: !!season,
	});
	...
	useEffect(() => {
		if (season) {
			reset({
				scoreType: season.scoreType === "elo-individual-vs-team" ? "elo" : season.scoreType,
				name: season.name,
				slug: season.slug,
				initialScore: season.initialScore,
				kFactor: season.kFactor,
				rounds: season.rounds ?? 1,
	...
	// Check slug availability using dedicated endpoint (only if slug changed)
	const { data: slugCheck } = useQuery({
		queryKey: ["season-slug-check-edit", debouncedSlug, season?.slug],
		queryFn: async () => {
			if (!debouncedSlug) return { available: true };
			// If slug hasn't changed from original, it's available
			if (season && debouncedSlug === season.slug) return { available: true };
			return await trpcClient.season.checkSlugAvailability.query({ slug: debouncedSlug });
		},
		enabled: !!debouncedSlug && !!season,
	});

	const isSlugTaken = slugCheck ? !slugCheck.available : false;
	const canSubmit = !editMutation.isPending && !isSlugTaken;

	const onSubmit = async (values: EditSeasonFormValues) => {
		setApiError("");
		...
		try {
			...
			// Only include score-related fields if no matches exist
			if (!hasMatches) {
				if (isElo) {
					input.initialScore = values.initialScore;
					input.kFactor = values.kFactor;
				}
				// Note: scoreType cannot be changed via edit endpoint
			}
					...
					<form onSubmit={handleSubmit(onSubmit)} className="space-y-2 p-1">
							...
							{scoreTypes.map((type) => {
								...
								return (
									<button
										key={type}
										type="button"
										onClick={() => !hasMatches && setValue("scoreType", type)}
										disabled={hasMatches}
										className={`
                      ...
                      `}
						...
						<FieldGroup className="space-y-2">
							...
							{/* ELO Configuration - Read only if matches exist */}
							{isElo && (
								<div className="grid grid-cols-2 gap-3">
									<Field>
										<FieldLabel className="font-mono text-xs font-medium tracking-wide mb-0.5">
										...
										<Input
											...
											type="number"
											disabled={hasMatches}
											className="h-8 font-mono focus:border-amber-500 focus:ring-amber-500/20 text-sm disabled:opacity-50"
										/>
							...
							{/* Rounds - only for 3-1-0 scoring, read only if matches exist */}
							{!isElo && (
								<Field>
									<FieldLabel className="font-mono text-xs font-medium tracking-wide mb-0.5">
										Rounds
									...
									<Input
										...
										disabled={hasMatches}
										placeholder="1"
										className="h-8 font-mono focus:border-blue-500 focus:ring-blue-500/20 text-sm disabled:opacity-50"
									/>
