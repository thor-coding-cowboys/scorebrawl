...
import type { SeasonPlayerStandingDTO } from "@scorebrawl/database/dto";
...
const schema = z.object({
  awayPlayers: z.array(z.any()).min(1),
  homePlayers: z.array(z.any()).min(1),
...
export const MatchForm = ({ leagueSlug, seasonSlug }: MatchFormProps) => {
  ...
  const { data: seasonPlayers } = trpc.seasonPlayer.getStanding.useQuery({
    leagueSlug,
    seasonSlug,
  });
  const utils = trpc.useUtils();
  const { mutate, isPending } = trpc.match.createEloMatch.useMutation();

  const [teamSelection, setTeamSelection] = useState<PlayerWithSelection[]>([]);

  useEffect(() => {
    setTeamSelection((seasonPlayers as PlayerWithSelection[]) ?? []);
  }, [seasonPlayers]);
  ...
  const onSubmit = async ({ homeScore, homePlayers, awayPlayers, awayScore }: FormValues) => {
    mutate(
      ...
      },
      {
        onSuccess: () => {
          form.reset();
          setTeamSelection(seasonPlayers ? [...(seasonPlayers as PlayerWithSelection[])] : []);
  ...
  if (season.scoreType !== "elo") {
    navigate({
      ...
      params: { leagueSlug },
      search: { errorCode: "UNSUPPORTED_SCORE_TYPE" },
    });
    return null;
  }
  ...
  return (
    <DrawerContent>
      <div className="mx-auto w-full max-w-xl px-4">
        ...
        <div className="flex max-h-[70vh] justify-around overflow-y-auto">
