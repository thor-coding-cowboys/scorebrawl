...
const AchievementsCell = ({
  ...
  const { data } = trpc.achievement.getUserAchievements.useQuery({ leagueSlug, leaguePlayerId });
...
export const LeaguePlayersTable = ({
  ...
  const { data } = trpc.leaguePlayer.getAll.useQuery({ leagueSlug });
        ...
        {data?.map((player) => {
          ...
          return (
            <TableRow key={player.leaguePlayerId}>
              <TableCell>
                <Link
                  to="/leagues/$leagueSlug/players/$playerId"
                  params={{ leagueSlug, playerId: player.leaguePlayerId }}
