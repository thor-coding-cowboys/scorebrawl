...
const TopPlayerCell = ({ seasonSlug, leagueSlug }: { seasonSlug: string; leagueSlug: string }) => {
  const { data } = trpc.seasonPlayer.getTop.useQuery({ leagueSlug, seasonSlug });
...
const TopTeamCell = ({ seasonSlug, leagueSlug }: { seasonSlug: string; leagueSlug: string }) => {
  const { data } = trpc.seasonTeam.getTop.useQuery({ leagueSlug, seasonSlug });
  if (!data) {
    return null;
  }
  return (
...
const ActionCell = ({
  ...
  const updateClosedStatus = trpc.season.updateClosedStatus.useMutation({
    onSuccess: () => {
...
export const SeasonTable = ({
  ...
  const { data, isLoading } = trpc.season.getAll.useQuery({ leagueSlug });
                ...
                <TableRow
                  ...
                  onClick={() =>
                    navigate({
                      ...
                      params: { leagueSlug, seasonSlug: season.slug },
                    })
                  }
                >
                  <TableCell>
